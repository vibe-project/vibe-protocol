...
// This function is exposed to the module's `transport.createHttpServer` as a
// constructor of transport server which consumes HTTP request-response exchange
// and produces transport.
    ...
    self.handle = function(req, res) {
        ...
        case "POST":
            ...
            req.on("end", function() {
                ...
                var transport = transports[req.params.id];
                if (transport) {
                    // For now only text message is supported.
...
function createStreamTransport(req, res) {
    ...
    // Likewise some host objects in old browsers need junk data to make
    // themselves be aware of change of response. Prints the padding and the
    // first message following `text/event-stream`.
    res.write(text2KB + "\ndata: " + uri + "\n\n");
    return self;    
}
...
function createLongpollTransport(req, res) {
    ...
    self.send = function(data, noQueue) {
        ...
        if (response) {
            ...
            // For others, no formatting is needed. And, all the long polling
            // transports have to finish the request after processing. The

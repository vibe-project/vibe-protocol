...
module.exports = function(uri, options) {
    ...
    if ((urlObj.protocol === "http:" || urlObj.protocol === "https:") && urlObj.query.transport === "stream") {
        ...
        self.connect = function() {
            ...
            .on("error", onerror)
            // If the underlying connection was closed
            .on("close", function() {
                self.emit("close");
            })
            .on("response", function(res) {
                ...
                // On a message of the event stream format of Server-Sent
                // Events,
                function onmessage(data) {
                    if (!handshaked) {
                        ...
                        // `code` is a first character of a message and used to
                        // recognize that delivered message is text message or
                        // binary message.
                        var code = data.substring(0, 1);
                        data = data.substring(1);
                        switch (code) {
                        // If the `code` is `1`, the remainder of message is a
                        // plain text message. Fires `text` event.
                        case "1":
                            self.emit("text", data);
                            break;
                        // If the `code` is `2`, the remainder of message is a
                        // Base64-encoded binary message. Decodes it in Base64
                        // and fires `binary` event.
                        case "2":
                            self.emit("binary", new Buffer(data, "base64"));
                            break;
                        // Otherwise, it is invalid. Fires an error and closes
                        // the connection.
                        default:

...
function createLongpollTransport(req, res) {
    ...
    self.refresh = function(req, res) {
        ...
        // When the complete response has been sent.
        res.on("finish", function() {
            // If this request was to `poll` and this response is consumed,
            // completion of this response is the end of the transport. Hence,
            // fires the `close` event.
            if (req.params.when === "poll" && !res.consumed) {
                self.emit("close");
        ...
        // A custom property to mark whether or not this response is consumed to
        // send a message. A consumed response can't be used again.
        res.consumed = false;
        // A custom method to send a message through this response.
        res.endWithMessage = function(data) {
            // Flags the current response is consumed.
            res.consumed = true;
            // `data` should be either a `Buffer` or a string.
            if (typeof data === "string") {
        ...
        case "open":
            ...
            // perform `longpoll` transport can't read response headers as well
            // as write request headers. That's why we uses the first response's
            // body as a handshake output instead of headers. The handshake
            // result should be formatted in URI. And transport id should be
            // added as `id` param.
        ...
        case "poll":
            ...
            // so that the server should send them again.
            if (queue.length) {
                // Removes the first cached message from the queue and send it.

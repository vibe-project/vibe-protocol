...
function server(options) {
    ...
    options.transports = options.transports || ["ws", "sse", "streamxhr", "streamxdr", "streamiframe", "longpollajax", "longpollxdr", "longpolljsonp"];
    // A heartbeat interval in milliseconds.
    options.heartbeat = options.heartbeat || 20000;
    // This is just to speed up heartbeat test. It means the time to wait for
    // the server's response. The default value is `5000`.
        ...
        case "GET":
            ...
            case "open":
                ...
                case "sse": case "streamxhr": case "streamxdr": case "streamiframe":
                    var s = socket(req.params, streamTransport(req, res));
                    s.uri = req.url;
                    server.emit("socket", s);
                    break;
                case "longpollajax": case "longpollxdr": case "longpolljsonp":
                    var s = socket(req.params, longpollTransport(req, res));
                    s.uri = req.url;
                    server.emit("socket", s);
                    break;
    ...
    server.handleUpgrade = function(req, sock, head) {
        ...
        webSocketFactory.handleUpgrade(req, sock, head, function(ws) {
            ...
            // using it.
            var s = socket(req.params, wsTransport(ws));
            s.uri = req.url;
            server.emit("socket", s);
        });
    };

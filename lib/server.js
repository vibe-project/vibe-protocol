...
function server(options) {
    ...
    // These options are to negotiate the protocol. A client will connect to
    // the server following them.
    options = options || {};
    // A set of supported transports to be used by a client.
    options.transports = options.transports ||
        ["ws", "sse", "streamxhr", "streamxdr", "streamiframe", "longpollajax", "longpollxdr", "longpolljsonp"];
    // A heartbeat interval in milliseconds.
    options.heartbeat = options.heartbeat || 20000;
    // This is just to speed up heartbeat test and not required generally. It
    // means the time to wait for the server's response. The default value is
    // `5000`.
    if (!options._heartbeat) {
        options._heartbeat = 5000;
    }
        ...
        case "GET":
            ...
            case "handshake":
                ...
                var result = JSON.stringify({
                    // An identifier for a socket the client will establish. It
            ...
            // `socket` event. `transport` param is an id of transport the
            // client uses.
            case "open":
            ...
            break;
        // `POST` method is used to supply HTTP transport with message as a
    ...
    function socket(params, transport) {
        ...
        socket.send = function(type, data, resolved, rejected) {
            ...
            // For `reply` extension, stores resolved and rejected callbacks if
            // they are given.
            if (event.reply) {
...
transports.ws = function(ws) {
    // It delegates WebSocket's events to transport and transport's behaviors to
    // WebSocket.
...
transports.sse = function(req, res) {
    ...
    // When either client or server closes the transport, fires the `close`
    // event.
    function onclose() {
        if (onclose.done) {
            return;
    ...
    // `close` event will be fired. Don't do that by yourself.
    transport.close = function() {
        res.end();
    };
    return transport;
...
transports.longpollajax = function(req, res) {
    ...
    // then `ended` is also true but not vice versa.
    var written;
    ...
    // In long polling, an exchange of request and response is disposable so
    // expose this method to supply with subsequent exchanges.
    transport.refresh = function(req, res) {
        // The content-type header should be `text/javascript` for
        // `longpolljsonp` and `text/plain` for the others.
        res.setHeader("content-type", "text/" + 
        ...
        if (req.params.when === "open") {
            res.end();
        // If the request is to `poll`, remove the client-received events from
        // queue and flush the rest in queue if they exist.
        } else {
            // Resets the response, flags, timers as new exchange is supplied.
            response = res;
            ended = written = false;
            clearTimeout(closeTimer);
            // If aborted is `true`, it means the user aborted the connection
            // but it couldn't be done because the current response is already
            // ended for other reason. So ends the new exchange.
            if (aborted) {
                res.end();
                return;
            }
            ...
            // FYI, `a.splice(b, 1)` in JavaScript means removing `b` from `a`.
    ...
    transport.close = function() {
        ...
        // will be ended immediately by `aborted` flag so it will fire the
        // `close` event. So you don't need to manually dispatch the close event
        // here.
        if (!ended) {
            response.end();
        }

...
// [Vibe protocol](http://atmosphere.github.io/vibe/projects/vibe-protocol/) written in
// easy-to-read JavaScript running on Node.js.
// 
// **Note**
// * For production use, see the Vibe Java Server.
...
// ## API
// `require("vibe-protocol").server()` creates a vibe server that is installed
// by passing request and upgrade events dispatched by Node's HTTP/HTTPS server
// to the server.
//
...
transports.stream = function(req, res) {
    ...
    // `streamxhr` in Android browser lower than 3.
    res.write(text2KB + "\n");
    ...
    transport.send = function(data) {
        ...
        var payload =
            ...
            // or `\r\n` but because data is JSON, it's not needed. So prepend
            // 'data: ' and append `\n\n` to the data.
            "data: " + data + "\n\n";
...
transports.longpoll = function(req, res) {
    ...
    transport.refresh = function(req, res) {
        ...
        function onclose() {
            ...
            if (req.params.when === "poll" && !written) {
                transport.emit("close");
            // Otherwise sets a timer to fire close event between polls.
            // Without the timer, if the client disconnects connection
            // during dispatching event, this connection will remain in limbo.
            } else {
        ...
        // and flush the rest in queue if they exsits or wait the next data.
            ...
            // clear the timer and end the new exchange.
            if (aborted) {
                res.end();
                return;
            }
    ...
    transport.close = function() {
        ...
        aborted = true;
        // By ending response if possible. If it's not possible, a next poll
        // request will be ended immediately thanks to `aborted` flag so it will
        // fire the close event. So you don't need to manually dispatch the close event here.
        if (!ended) {
            response.end();
        }
    };        
...
// ## Socket
// A socket is a connectivity between the two vibe endpoints and an interface
// for developers creating vibe applications.
//
// **Events**
...
function socket(params, transport) {
    ...
    socket.id = params.id;
    // An auto-increment id for event.
    var eventId = 0;
    ...
    socket.send = function(type, data, resolved, rejected) {
        ...
        // * `reply: boolean`: true if this event requires the reply.
        var event = {
                // In case of long polling, these ids are echoed back as a query
                // string to the URL in GET. To avoid `414 Request-URI Too Long`
                // error, though it is really not that important, it would be
                // better to use small sized id.
                id: "" + eventId++, 
                type: type, 
                data: data, 

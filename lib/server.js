...
module.exports = function() {
    ...
    // is needed to be retrieved using identifier. An identifier is set after
    // handshake.
    var httpTransports = [];
        ...
        case "GET":
            ...
            case "open":
                ...
                if (transport) {
                    ...
                    t.on("close", function() {
                        httpTransports.splice(httpTransports.indexOf(this), 1);
                    });
            ...
            case "poll":
                ...
                var transport = httpTransports.filter(function(t) {return req.params.id === t.id;})[0];
                if (transport) {
...
transports.stream = function(req, res) {
    ...
    // Because `stream` transport consists of multiple exchanges, an identifier
    // is required. UUID is recommended as the identifier format.
    self.id = uuid.v4();
...
transports.longpoll = function(req, res) {
    ...
    // Because `longpoll` transport consists of multiple exchanges, an
    // identifier is required. UUID is recommended as the identifier format.
    self.id = uuid.v4();
    ...
    self.refresh = function(req, res) {
        ...
        res.on("finish", onclose);
        // If the request is to `open`, it will be used to deal with handshake.
    ...
    self.handshake = function(map) {
        ...
        // A handshake result is URI.
        var uri = url.format({query: map});
        // The first message is the handshake result and should be not formatted
        // and cached.
        res.end(uri);
    };
    return self;

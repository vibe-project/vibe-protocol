...
transports.stream = function(req, res) {
    ...
    // than 1KB, be composed of white space character.
...
transports.longpoll = function(req, res) {
    ...
    var uri = url.format({query: {id: self.id}});
    // In case of `longpoll`, there is no need to format and cache the handshake
    // result because it is the first response.
    response.end(params.jsonp === "true" ?

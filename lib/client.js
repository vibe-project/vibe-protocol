...
transports.longpoll = function(uri) {
    ...
    .on("error", onerror).on("response", function(res) {
        ...
        function onresponse(res) {
            ...
            .on("end", function() {
                if (body) {
                    // The message format is `id|data`. For example, `45|Hi`
                    // tells `45` is message id and `Hi` is message data. Here
                    // `match[1]` is the message id and `match[2]` is the
                    // message data.
                    var match = /(\d+)\|(.*)/.exec(body);
                    if (!handshaked) {
                        handshaked = true;
                        // The first message to complete handshake contains an
                        // id of this transport. Therefore, exceptionally `open`
                        // event should dispatch before starting polling.
        ...
        // Deal with the first response from the server.
        onresponse(res);

        function poll(lastMsgId) {
            // From the second request, `when` param should be `poll` and
            // `lastMsgId` should be provided for ACK that is a message id of
            ...
            // reading body.
            .on("error", onerror).on("response", onresponse);
        }
    });
    self.abort = function() {

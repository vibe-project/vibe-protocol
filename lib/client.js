...
function socket(uri, options) {
    ...
    http.get(url.parse(buildURI(uri, {when: "handshake"})))
    .on("error", function() {
        socket.emit("close");
    })
    .on("response", function(res) {
        ...
        res.on("end", function() {
            // The result of handshaking is a form of JSON.
            var result = JSON.parse(body);
            // A newly issued id for this socket.
            socket.id = result.id;
            // Options that a user assigned explicitly are prioritized than 
            // the result of handshaking, but it's usually for testing.
            if (!options.transport) {
                ...
                // If nothing is available, it's not possible to connect to the server.
                // Fire a close event.
                if (candidates.length === 0) {
                    socket.emit("close");
                    return;
                }
                // Choose the first one among transport both client and server support.
                options.transport = candidates[0];
            }
            options.heartbeat = options.heartbeat || result.heartbeat;
            options._heartbeat = options._heartbeat || result._heartbeat || 5000;
            // According to the transport option, create the transport and connect
            // to the server.
            transport = transports[options.transport](uri, {
                id: socket.id, 
            ...
            transport.on("message", function(text) {
                ...
                // the corresponding event will receive reply controller as 2nd argument. 
                // It calls the server's resolved or rejected callback by sending `reply`
                // event.
                } else {
    ...
    socket.close = function() {
        ...
        // would be null. Then, fire the close event. If not, the transport will
        // fire the close event and it will propagate to this socket.
        if (transport) {
            transport.close();
        } else {
            socket.emit("close");
    ...
    socket.on("open", function() {
        // The option `_heartbeat` is just to speed up heartbeat test. It means
        // the time to wait for the server's response. The default value is
        // `5000`.

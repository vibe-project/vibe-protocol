...
module.exports = function(uris, options) {
    ...
    self.open = function() {
        ...
        for (var i = 0; i < uris.length; i++) {
            ...
            // URI whose scheme is `http` or `https` and `transport` param is
            // absent is an abbreviated one. No transport stands for `ws`,
            // `stream` and `longpoll` transport for the sake of convenience.
            // For example, if uri is `http://localhost:8080/vibe?transport=ws`,
            // it's replaced with
        ...
        function init(trans) {
            ...
            transport.on("text", function(text) {
                ...
                // 
                // To implement `reply` extension, the following properties
                ...
                // To implement `reply` extension, provides a reply controller
                // to an event handler.
                } else {
                    var latch;
...
transports.httpbase = function(uri, options) {
    ...
    self.send = function(data) {
        ...
        // `&` can be preceded safely, and also because transport id is UUID, no
        // need to call `encodeURIComponent`.
        reqOpts.path += "&id=" + self.id;
        ...
        .on("error", onerror).on("response", function(res) {
            ...
            .on("end", function() {
                ...
                function poll() {
                    ...
                    req = http.get(uri + "&id=" + self.id + "&when=poll")

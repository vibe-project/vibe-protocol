...
transports.longpoll = function(uri) {
    ...
    .on("error", onerror).on("response", function(res) {
        ...
        .on("end", function() {
            ...
            // of handshaking.
            self.emit("open", body);
            // After setting the transport id that is done in dispatching `open`
            // event, starts polling.
            poll();
        });
        ...
        function poll(lastMsgId) {
            ...
            // the preceding request. The first request's message, handshaking
            // result, has no id.

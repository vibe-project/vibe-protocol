...
// This function is exposed to this module's `open` as a function to create
...
module.exports = function(uris, options) {
    ...
    var transport;
    // Translates URI, creates and associates a transport with the socket.
    self.open = function() {
        // If `uris` is a string, makes it array. FYI, `[].slice.call(uris)`
        // returns a copied `uris`.
        uris = Array.isArray(uris) ? [].slice.call(uris) : [uris];
        ...
        function open() {
            // If there is no remaining transport, fires `error` and `close`
            // event as it means that all available transports failed.
            if (uris.length === 0) {
                self.emit("error", new Error());
                self.emit("close");
                return;
            }
            // Removes the first element and gets it. For example,
            // `[1,2].shift()` results in `[2]`.
            var uri = uris.shift();
            var transportName;
            // Determines the transport name from URI.
            ...
            trans.on("close", open);
            // At the socket level, the first message is used to handshake.
            // `once` registers one-time event handler.
            trans.once("text", function(text) {
                // The handshake output is in the form of URI and uses query
                // part to get/set header.
                var result = url.parse(text, true).query;
                // To maintain alive connection, heartbeat is used.
                options.heartbeat = +result.heartbeat;
                // `_heartbeat` is usually for testing so it may be not passed
                // from the server. The default value is `5000`.
                options._heartbeat = +result._heartbeat || 5000;
                // Now that handshaking is completed, removes `close` event's
                // `open` handler because it's only to do fallback within
                // `transports` option.
                trans.removeListener("close", open);
                // The working transport is found so initializes it.
                init(trans);
                // And fires `open` event which is the first event user can
                // handle.
                self.emit("open");
            });
...
// loss and detecting disconnection. Each transport has its own URI format and
// handles only valid URI in accord with that format.
var transports = {};
...
// connection. URI's protocol should be `ws` or `wss`.
...
transports.httpbase = function(uri, options) {
    ...
    // For the client to send data to the server 
    self.send = function(data) {
        var reqOpts = url.parse(uri);
        // `id` should be added to query. As it has already `transport` param,
        // `&` can be preceded safely.
...
// response and the server prints chunk as data to response. URI's protocol
// should be `http` or `https` and transport param should be `stream`.
transports.stream = function(uri, options) {
    ...
    self.connect = function() {
        ...
        // param should be `open`. In case of Server-Sent Events, `sse` param
        // should be `true`.
...
// and again. URI's protocol should be `http` or `https` and transport param
// should be `longpoll`.
transports.longpoll = function(uri, options) {
    ...
    self.connect = function() {
        ...
        // request's `when` param should be `open`. In case of JSONP, `jsonp`
        // param should be `true` and `callback` param should be provided as well.
        req = http.get(uri + "&when=open")
        .on("error", onerror).on("response", function(res) {
            ...
            .on("end", function() {
                ...
                // `open` event, polling must be started. Because, when a user
                // closes the transport on open event, the active `req` is
                // required.
                poll();
                self.emit("open");
                // Then starts polling.
                function poll() {
                    // From the second request, `when` param should be `poll`
                    // and `id` param should be added.
                    ...
                    .on("error", onerror).on("response", function(res) {
                        ...
                        .on("end", function() {
                            // The response's body is message.
                            if (body) {
                                // Starts a poll request again before to fire
                                // `text` event. There must be no idle time
                                // between polling.
                                poll();
                                self.emit("text", body);
    ...
    self.abort = function() {
        ...
        // the server by default. But, that is a normal case of close in long
        // polling, hence removes all default error handlers.
        req.removeAllListeners("error");
        // To fire `close` event, registers `error` event handler to `req`. The
        // above `response` event handler is called only when response is
        // returned from the server so that `close` event is called only once.

...
module.exports = function(uri, options) {
    ...
    // A set of transport id. It is used to connect to the server in order.
    options.transports = options.transports || ["ws", "stream", "longpoll"];
    // A transport timeout in ms. A transport should establish a connection
    // during that time.
    options.timeout = options.timeout || 3000;
    // A reference of transport associated with this socket.
    var transport;
    // Try connection using `i+1`th transport of `transports` option. At the
    // first time, it starts with `0`.
    connect(0);
    function connect(i) {
        // When this attempt fails,
        function next() {
            // If next one is available, try with that.
            if (i < options.transports.length) {
                i++;
                connect(i);
            // If not, fire `error` and `close` event as it means that all
            // available transports failed to establish a connection.
            } else {
                self.emit("error", new Error()).emit("close");
            }
        }
        // `uri` is a kind of input for handshaking as well as indicates an
        // endpoint. As an option, `timeout` should be passed.
        var trans = transports[options.transports[i]](uri, {timeout: options.timeout});
        // If this transport has failed to establish a connection:
        trans.on("close", next);
        // If this transport has established a connection:
        trans.on("open", function() {
            // Removes `close` event's `next` handler because it's only to do
            // fallback within `transports` option. For reconnection, `connect`
            // should start with `0`.
            trans.removeListener("close", next);
            // As it establishes a connection, assigns this transport to one
            // associated with the socket and associates it with socket.
            transport = trans;
            // Socket's handshake is done through the first message via
            // transport.
            var handshaked = false;
            // When the transport has received a message from the server.
            transport.on("message", function(text) {
                // Only the first message is to handshake.
                if (!handshaked) {
                    handshaked = true;
                    ...
                    // Fires `open` event which is the first event user can
                    // handle.
                    self.emit("open");
                    return;
                }

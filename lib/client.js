...
["streamxhr", "streamxdr", "streamiframe"].forEach(function(tpName) {
    transports[tpName] = function(u, params) {
        ...
        .on("response", function(res) {
            ...
            // of a single event. This buffer is used to aggregate fragments to
            ...
            res.on("data", function(chunk) {
                ...
                // `data: {}\n\n` will be split into `["data: {}", ""]`.
                chunk.split(/\n\n/).forEach(function(line) {
                    // Fires a message event and clears the buffer.
                    if (!line) {
                        transport.emit("message", buffer);
                        buffer = "";
                    // A single data field, e.g. the former of `data: {...` and `...}`.
                    } else if (/^data:\s/.test(line)) {
                        buffer += line.substring("data: ".length);
                    // A fragment of a data field, e.g. the latter of `data: {...` and `...}`.
                    } else {
                        buffer += line;
                    }
                });
...
transports.longpollajax = function(u, params) {
    ...
    // they use `GET` method.
    req = http.get(uri(u, {id: params.id, when: "open", transport: "longpollajax", heartbeat: params.heartbeat}))

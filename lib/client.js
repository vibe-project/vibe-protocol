...
//     * [WebSocket](#websocket)
//     * [HTTP base](#http-base)
//     * [Streaming by Server-Sent Events](#streaming-by-server-sent-events)
//     * [Streaming by XMLHttpRequest, XDomainRequest and Hidden iframe](#streaming-by-xmlhttprequest-xdomainrequest-and-hidden-iframe)
//     * [Long polling by AJAX and XDomainRequest](#long-polling-by-ajax-and-xdomainrequest)
//     * [Long polling by JSONP](#long-polling-by-jsonp)
// 
// ---
...
// This module is exposed to the parent module's `open` as a constructor of
// client.
module.exports = socket;

// ## URI
// An URI specified in [RFC 3986](http://tools.ietf.org/html/rfc3986) is the
// complete path to the vibe server endpoint. The protocol uses the query string
// to pass information to interact with the server so be aware of reserved
// parameters or use RESTful path.
// 
...
// 
// Also it is expected to use HTTP and WebSocket client implementation that are
// standardized in [RFC 2616](http://tools.ietf.org/html/rfc2616) and [RFC
...
function socket(uri, options) {
    ...
    // Generates an UUID as an identifier of this socket. It should be
    // universally unique literally.
    socket.id = uuid.v4();
    
    // According to the transport option, create the transport and connect
    // to the server.
    var params = {id: socket.id, transport: options.transport, heartbeat: options.heartbeat};
    var transport = transports[options.transport](uri, params);
    ...
    // ### Receiving an event
    // When the underlying transport has received a message from the server.
    transport.on("message", function(text) {
        ...
        // 
        // If the client implements `receiving replyable event` extension, the
        ...
        // If the server sends a plain event not replyable event, dispatch it.
        if (!event.reply) {
        ...
        // controller as 2nd argument. It calls the server's resolved or 
    ...
    socket.send = function(type, data, resolved, rejected) {
        ...
        // * `data: any`: an event data.
        // 
        // If the client implements `sending replyable event` extension, the
...
transports.ws = function(uri, params) {
    ...
    var ws = new WebSocket(buildURI(uri, {
        id: params.id, 
        when: "open", 
        transport: "ws", 
...
transports.httpbase = function(uri, params) {
    ...
    transport.close = function() {
        ...
        http.get(url.parse(buildURI(uri, {id: params.id, when: "abort"})));
    };
    return transport;
};
...
transports.sse = function(uri, params) {
    ...
    var es = new EventSource(buildURI(uri, {
        id: params.id, 
        when: "open", 
        transport: "sse", 
        heartbeat: params.heartbeat
...
// ### Streaming by XMLHttpRequest, XDomainRequest and Hidden iframe
...
// client don't have to implement them.
...
// ### Long polling by AJAX and XDomainRequest
...
["longpollajax", "longpollxdr"].forEach(function(tpName) {
    ...
    transports[tpName] = function(uri, params) {
        ...
        // If the first request is completed normally since the server is available,
        // start to poll and fire the open event. There must be no idle time 
        // between the poll. Starting poll request is always prior to dispatching 
        // events.
        .on("response", function() {
            // For the first time, starts with empty array.
            poll([]);
            // The poll request is started so fires the open event.
            transport.emit("open");
            // #### Request to poll
            ...
            function poll(lastEventIds) {
                ...
                req = http.get(buildURI(uri, {
                    id: params.id, 
                    when: "poll", 
                    lastEventIds: lastEventIds.join(",")
                }))
                .on("error", function() {
                    transport.emit("close");
                })
                ...
                .on("response", function(res) {
                    ...
                    res.on("end", function() {
                        if (body) {
                            ...
                            // that the client couldn't receive before. FYI,
